        ...
        <div
          ...
          style={{
            ...
            background: spots
              .map(
                (x) =>
                  `radial-gradient(ellipse 25% 40% at ${x}% 68%, rgba(${lightColor},0.15) 0%, transparent 70%)`,
              )
              .join(", "),
          }}
        />
        ...
        <div
          ...
          style={{
            ...
            background: spots
              .map(
                (x) =>
                  `radial-gradient(ellipse 35% 30% at ${x}% 80%, rgba(${lightColor},0.06) 0%, transparent 60%)`,
              )
              .join(", "),
          }}
        />
      ...
      >
        {spots.map((pos, i) => (
          <motion.div
            ...
            transition={
              isFlickering ? { duration: 0 } : { delay: i * 0.1, duration: 0.8, ease: "easeInOut" }
            }
          ...
          >
            {canvasReady && (
              // If a light's WebGL context can't be created or is lost (weak
              // GPU, too many live contexts), this cone quietly drops to
              // nothing — the CSS-gradient light layer beneath (zIndex 15)
              // still lights the room, and nothing takes the page down.
              <CanvasBoundary label="studio light">
                <Canvas
...
export const VolumetricStudio = ({
  ...
  return (
    <section
      ref={sectionRef}
      className={cn(
        "relative w-full h-full min-h-[600px] bg-black overflow-hidden font-sans",
        className,
      )}
    >
      <Room
