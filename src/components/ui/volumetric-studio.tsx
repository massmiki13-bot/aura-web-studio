...
// Deep import instead of the package barrel: the full `@react-three/drei`
// entry re-exports its entire surface (physics, postprocessing, etc.) and
// crashes esbuild's dependency pre-bundling in this project. Importing the
// single component file avoids pulling that whole graph in.
...
type RoomProps = {
  ...
  className?: string;
  /** Only mount the WebGL spotlight canvases once the studio is actually
   *  near the viewport — three separate `<Canvas>` contexts are the
   *  heaviest part of this component, and it can sit well below the fold. */
  canvasReady?: boolean;
};
...
function Room({
  ...
  return (
    <div aria-hidden className={`absolute inset-0 overflow-hidden bg-black pointer-events-none ${className}`}>
      <div
        className="absolute inset-0"
        style={{ clipPath: poly([tl, tr, br, bl]), background: "linear-gradient(to bottom, rgba(20,20,22,1) 0%, rgba(8,8,10,1) 100%)" }}
      />
      <div
        className="absolute inset-0"
        style={{ clipPath: poly([[0, 0], [100, 0], tr, tl]), background: "linear-gradient(to bottom, rgba(0,0,0,1) 0%, rgba(0,0,0,0.85) 100%)" }}
      />
      <div
        className="absolute inset-0"
        style={{
      ...
      <div
        className="absolute inset-0"
        style={{ clipPath: poly([[0, 100], [100, 100], br, bl]), background: "linear-gradient(to top, rgba(15,15,17,1) 0%, rgba(6,6,8,1) 100%)" }}
      />
      ...
      <div
        ...
        style={{ zIndex: 15, opacity: lightsOn ? intensity : 0, transition: isFlickering ? "none" : `opacity 700ms ${EASE}`, mixBlendMode: "screen", willChange: "opacity" }}
      >
        <div
          className="absolute inset-0"
          style={{ clipPath: poly([tl, tr, br, bl]), background: spots.map((x) => `radial-gradient(ellipse 25% 40% at ${x}% 68%, rgba(${lightColor},0.15) 0%, transparent 70%)`).join(", ") }}
        />
        <div
          className="absolute inset-0"
          style={{ clipPath: poly([[0, 0], tl, bl, [0, 100]]), background: `radial-gradient(ellipse 40% 50% at 15% 75%, rgba(${lightColor},0.08) 0%, transparent 60%)` }}
        />
        <div
          className="absolute inset-0"
          style={{ clipPath: poly([[100, 0], tr, br, [100, 100]]), background: `radial-gradient(ellipse 40% 50% at 85% 75%, rgba(${lightColor},0.08) 0%, transparent 60%)` }}
        />
        <div
          className="absolute inset-0"
          style={{ clipPath: poly([[0, 100], [100, 100], br, bl]), background: spots.map((x) => `radial-gradient(ellipse 35% 30% at ${x}% 80%, rgba(${lightColor},0.06) 0%, transparent 60%)`).join(", ") }}
        />
      </div>
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    boxShadow: lightsOn ? `0 0 20px 8px rgba(255,255,255,0.9), inset 0 0 8px #fff` : `inset 0 2px 5px rgba(0,0,0,0.9), inset 0 -1px 1px rgba(255,255,255,0.05)`,
                  }}
                />
              </div>
              <div
      ...
      <div className="absolute pointer-events-none w-full h-[80px] bg-linear-to-b from-black/60 to-transparent blur-xl" style={{ zIndex: 29, top: "4%", left: 0 }} />
      <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 30, clipPath: poly([[0, 0], [100, 0], tr, tl]) }}>
        <div
      ...
      <div
        className="absolute inset-0"
        style={{ zIndex: 20, background: `radial-gradient(ellipse 90% 80% at 50% 45%, transparent 55%, rgba(0,0,0,${vignette}) 100%)` }}
      />
      <div
        className="absolute inset-0 pointer-events-none"
        style={{ zIndex: 25, opacity: 0.04, mixBlendMode: "screen", backgroundImage: GRAIN_NOISE, backgroundSize: "256px 256px" }}
      />
    </div>
  );
}

export const VolumetricStudio = ({ className, children }: { className?: string; children?: React.ReactNode }) => {
  const [lightsOn, setLightsOn] = useState(false);
  const [isFlickering, setIsFlickering] = useState(true);
  const [canvasReady, setCanvasReady] = useState(false);
  const sectionRef = useRef<HTMLElement>(null);

  // The three spotlight <Canvas> contexts are the heaviest part of this
  // component — defer mounting them until the studio is actually near the
  // viewport instead of the moment the (possibly below-the-fold) section
  // mounts. Plain scroll-driven bounding-rect check, not
  // IntersectionObserver, which doesn't fire reliably in this project.
  useEffect(() => {
  ...
  return (
    <section
      ref={sectionRef}
      className={cn("relative w-full h-full min-h-[600px] bg-black overflow-hidden font-sans", className)}
    >
      <Room
        lightsOn={lightsOn}
        intensity={1}
      ...
      />
      {/* Room's internal layers (light cones, vignette, grain, fixtures) use
          z-index up to 31 to stack correctly among themselves, so the actual
          content needs to sit above all of them to read as "in front of the
          light" rather than washed out underneath it. */}
      <div className="relative z-[40] w-full h-full pointer-events-none">{children}</div>
    </section>
  );
};
